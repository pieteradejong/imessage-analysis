...
class DatabaseConnection:
    ...
    def __init__(self, config: Config, *, use_memory: bool = False):
        ...
        if not config.validate():
            raise ValueError(
                f"Database file not found or not readable: {config.db_path_str}"
            )
    ...
    def get_columns_for_table(self, table_name: str) -> List[Tuple[str, str, int, Optional[str], Optional[int], int]]:
        """
    ...
    def get_row_counts_by_table(self, table_names: Optional[List[str]] = None) -> List[Tuple[str, int]]:
        """
    ...
    def execute_query(self, query: str, parameters: Optional[Tuple[Any, ...]] = None) -> List[Tuple[Any, ...]]:
        """
